...
const LaProductCard = ({ item }) => {
  return (
    <div className="pb-4 shadow-lg max-w-[350px] font-sans rounded-xl space-y-6 mx-auto bg-white transition-transform duration-300 transform hover:scale-105">
      <div className="flex justify-center w-full h-48 lg:h-[200px] relative">
        <div className="flex justify-between items-center left-4 right-4 top-4 absolute">
          ...
          <button className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 duration-200 text-white font-medium px-3 py-1 rounded-md">
            <span className="text-green-100 text-sm">{item?.discount_price}</span>% off
          </button>
        </div>
        <Image
          width={300}
      ...
      <div className="flex items-center justify-center flex-wrap gap-6 text-sm md:text-base">
        <button className="duration-300 text-purple-700 hover:scale-105 hover:text-white font-bold py-2 px-4 rounded border border-purple-700 hover:from-purple-700 hover:to-blue-700 hover:bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">
